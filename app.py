...
# 사용자 추가 함수
def add_user(userid, passwd, email, user_type):
    c.execute('INSERT INTO users (userid, passwd, email, user_type) VALUES (?, ?, ?, ?)', (userid, passwd, email, user_type))
    conn.commit()

# 사용자 정보 가져오기 함수
def get_users():
    c.execute('SELECT userid, passwd, email, user_type FROM users')  # 모든 열을 명시적으로 선택
    return c.fetchall()
...
with t1:
    ...
    if st.session_state['login_status']:
        st.success(f'로그인 성공: {st.session_state["current_user"]}')
        if st.button('로그아웃'):
            ...
            if st.button("비밀번호 찾기"):
                if check_email_exists(email):
                    ...
                    if result:
                        ...
                        # 이메일로 비밀번호 전송
                        send_email(email, "비밀번호 찾기", f"당신의 비밀번호는 {user_password}입니다.")
    ...
    with t2:
        if st.session_state['login_status']:
            ...
            with st.form('회원가입'):
                userid = st.text_input('아이디 (4자 이상)', key='ruserid')
                passwd = st.text_input('비밀번호 (6자 이상)', key='rpasswd', type='password')
                confirm_passwd = st.text_input('비밀번호 확인', key='rconfirm_passwd', type='password')
                email = st.text_input('이메일', key='remail')
                user_type = st.radio('사용자 유형을 선택하세요', ('학생', '선생님'))  # 사용자 유형 선택 추가
                    ...
                    if all([userid, passwd, confirm_passwd, email]):
                        ...
                        # Check email format
                            ...
                            if user:
                                ...
                                add_user(userid, passwd, email, user_type)  # 사용자 유형 추가
                                st.success('등록 성공')
    ...
    with t3:
        if st.session_state['login_status'] and st.session_state['current_user'] == 'admin':
            ...
            users = get_users()
            if users:
                df = pd.DataFrame(users, columns=['UserID', 'Password', 'Email', 'User Type'])  # Add user type column
                st.dataframe(df)
